...
#define GET_PHYSICAL_ADDRESS(frame, offset) ((frm_num << 8) | offset)
...
int main() {
    ...
    input_file = fopen(ADDRESS_FILE, "r");
    ...
    printf("Page Fault Rate = %.4f%%\n", (float)page_fault / total * 100);
    printf("TLB Hit Rate = %.4f%%\n", (float)(total - TLB_miss) / total * 100);
    fclose(input_file);
...
uint32_t get_add(uint32_t logical_add) {
    ...
    return GET_PHYSICAL_ADDRESS(frm_num, offset);
}
